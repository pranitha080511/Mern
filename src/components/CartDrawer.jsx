      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col h-full border-l border-pink-100">
          ...
          <div className="px-6 py-6 bg-black text-white flex justify-between items-center border-b border-gray-900">
            ...
            <button onClick={onClose} className="text-gray-400 hover:text-white transition duration-200" aria-label="Close cart">
              <X size={24} />
            </button>
          </div>
              ...
              <div className="flex-grow overflow-y-auto px-6 py-4 space-y-4">
                {cartItems.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-4 py-12">
                    ...
                    <button onClick={onClose} className="bg-pink-600 hover:bg-pink-700 text-white font-medium px-6 py-2 rounded-full text-sm shadow-md transition duration-200">
                      Continue Shopping
                    </button>
                  </div>
                ) : (
                    ...
                    <div key={item.id} className="flex items-center space-x-4 p-3 bg-pink-50/40 hover:bg-pink-50/70 rounded-xl border border-pink-100/50 transition duration-150">
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 bg-white hover:bg-gray-100 rounded-md border border-gray-200 text-gray-600 hover:text-black transition"><Minus size={12} /></button>
                          <span className="text-sm font-medium w-6 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1 bg-white hover:bg-gray-100 rounded-md border border-gray-200 text-gray-600 hover:text-black transition"><Plus size={12} /></button>
                        </div>
                      </div>

                      <button onClick={() => removeItem(item.id)} className="text-gray-400 hover:text-red-500 p-2 transition" aria-label={`Remove ${item.name} from cart`}>
                        <Trash2 size={18} />
                      </button>
                    </div>
                  ))
              ...
              {cartItems.length > 0 && (
                <div className="border-t border-gray-100 px-6 py-6 space-y-4 bg-gray-50">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    <button onClick={clearCart} className="w-full bg-white hover:bg-gray-100 text-gray-700 border border-gray-300 font-medium py-3 rounded-xl text-sm transition duration-200">Clear Cart</button>
                    <button onClick={handleCheckout} className="w-full bg-pink-600 hover:bg-pink-700 text-white font-medium py-3 rounded-xl text-sm shadow-lg hover:shadow-pink-600/35 transition duration-200">Checkout</button>
                  </div>
                </div>
              )}
            </>
